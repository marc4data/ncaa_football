...
def _rows(season: int, week, season_type: str, conference) -> pd.DataFrame:
    sql = """
...
def body(page) -> None:
    ...
    with states.section("srv_schedule"):
        ...
        states.render_or_state(
            ...
            renderer=lambda d: _grouped(d),
            fix_label="Clear filters", fix=filters.clear)


def _grouped(df: pd.DataFrame) -> None:
    ...
    for day, rows in df.groupby(df["game_date"], sort=True):
        ...
        table.render(rows, _columns(), caption="srv_schedule",
