...
# ── A196 (cfdb-main-R-2032): THE GAME TABLE MOVED TO `lib/schedule_table.py` ────────
#
# Today's "Looking Forward" is the same table, and a view may not import another
# view — so the columns and their cells now live in `lib/`, VERBATIM. These aliases
# keep every call site in this file unchanged, including the six the stacked card
# uses, so nothing about Schedule's rendering moved with them.
_text = schedule_table.text
_missing = schedule_table.missing
_team_name = schedule_table.team_name
_record_span = schedule_table.record_span
_team_with_record = schedule_table.team_with_record
_winner_side = schedule_table.winner_side
_winner_marker = schedule_table.winner_marker
_score_cell = schedule_table.score_cell
_weather_cell = schedule_table.weather_cell
_result_strip = schedule_table.result_strip
_neutral_glyph = schedule_table.neutral_glyph
_columns = schedule_table.columns
TEAM_NAME_MAX = schedule_table.TEAM_NAME_MAX
NEUTRAL_GLYPH = schedule_table.NEUTRAL_GLYPH
WINNER_GLYPH = schedule_table.WINNER_GLYPH
TIE_GLYPH = schedule_table.TIE_GLYPH
DOME_MARK = schedule_table.DOME_MARK
WEATHER_GLYPH = schedule_table.WEATHER_GLYPH
...
# A196: the query moved to `lib/schedule_table.py` so Today reads the same rows.
_rows = schedule_table.rows
...
# THE WEEKLY DISTRIBUTION BAND IS WITHDRAWN FROM THE PAGE, NOT DELETED (R-274).
#
# Marc, 2026-09-05: "take down the histograms on the schedule page... Cowork and I have been
# looking at other options and will ship requirements after we finish the Scores work,
# because I think that's where they will ultimately end up. I'm not a fan of how they look
# right now and we've got eyes on the site with the big day coming tomorrow." And, on scope:
# "Just take them from the presentation layer. We need the data behind it."
#
# SO NOTHING BEHIND IT MOVED. Four dbt models, two serving views, nine dbt tests and
# `lib/distribution.py` with its own suite all stay, and stay green. The renderer is now
# called by nothing — deliberately: it is a finished component waiting for a home, and
# rebuilding it later would cost far more than leaving it.
#
# What went was this page's plumbing: BAND_METRICS, the `_distributions` query, `_latest`,
# `_band_strip`, `_week_band` and `_season_to_date_band`. The CSS in theme.py is left where it
# is so the restore is a one-file change; no markup emits it today.
#
# THE WEEK GROUPING BELOW STAYS EVEN THOUGH THE BAND IS WHY IT EXISTS. Its docstring argued
# that grouping week-then-day rather than day-only kept the band from repeating per day. With
# the band gone the grouping is invisible — day order follows week order, so the output is
# exactly the day-only layout both views had before it. Removing it as well would be a second,
# unrequested change to a page under observation tonight.
...
def _by_week(df: pd.DataFrame, scope, render_day) -> None:
    """Day groups, in week then day order. Shared by both views so they cannot diverge."""
    _truncation_note(df)
    for _week, week_rows in df.groupby(df["week"], sort=True):
