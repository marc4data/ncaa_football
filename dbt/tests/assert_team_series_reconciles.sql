...
--
-- 1. WINS + WINS + TIES = GAMES. srv_matchup's first version derived one side as
...
--
-- 2. IT AGREES WITH srv_matchup. Two independent derivations of the same fact from the same
...
-- srv_matchup counts from the HOME team's perspective of each game, and carries the series
-- as it stood BEFORE that game. So its latest completed row plus that game's own result is
-- the all-time series.
against_serving as (
    select
        'srv_matchup' as check_name,
    ...
    where m.recency = 1
      -- games + unscored is MEETINGS, which is what srv_matchup counts. `games` alone is
